...
from PyQt5.QtWidgets import (QWidget, QApplication, QVBoxLayout, QPushButton,
                             QFrame, QLabel, QFrame, QGroupBox, QGridLayout, QTableWidget, QTableWidgetItem)
import random
import numpy as np
...
class FormCreate(QWidget):
    def __init__(self):
        ...
        self.hold = False # 真表示装有瓦砾，假表示没有装有瓦砾
        self.initUI()
    ...
    def holeinit(self):
        ...
        if (i == 0) and (j == 0):
            if (self.floorstatus[i+1][j]==0) and (self.floorstatus[i][j+1]==0) and (self.floorstatus[i+1][j+1]==0) and (self.floorstatus[i][j]==0):
            ...
            if (self.floorstatus[i][j-1]==0) and (self.floorstatus[i][j+1]==0) and (self.floorstatus[i+1][j-1]==0) and (self.floorstatus[i+1][j]==0)and (self.floorstatus[i+1][j+1]==0) and (self.floorstatus[i][j]==0):
                self.floorstatus[i][j]=1

        elif (j==0) and (i>0) and(i<7):
            if (self.floorstatus[i-1][j]==0) and (self.floorstatus[i+1][j]==0) and (self.floorstatus[i-1][j+1]==0) and (self.floorstatus[i][j+1]==0)and (self.floorstatus[i+1][j+1]==0) and (self.floorstatus[i][j]==0):
                self.floorstatus[i][j]=1

        elif (i==7) and(j>0) and(j<7):
            if (self.floorstatus[i][j-1]==0) and (self.floorstatus[i-1][j+1]==0) and (self.floorstatus[i-1][j]==0) and (self.floorstatus[i-1][j-1]==0)and (self.floorstatus[i][j+1]==0) and (self.floorstatus[i][j]==0):
                self.floorstatus[i][j]=1

        elif (j==7) and(i>0) and(i<7):
            if (self.floorstatus[i-1][j-1]==0) and (self.floorstatus[i][j-1]==0) and (self.floorstatus[i+1][j-1]==0) and (self.floorstatus[i+1][j]==0)and (self.floorstatus[i-1][j]==0) and (self.floorstatus[i][j]==0):
                self.floorstatus[i][j]=1

        else:
            if (self.floorstatus[i-1][j-1]==0) and (self.floorstatus[i-1][j]==0) and (self.floorstatus[i-1][j+1]==0) and (self.floorstatus[i][j-1]==0)and (self.floorstatus[i][j+1]==0) and (self.floorstatus[i+1][j-1]==0)and (self.floorstatus[i+1][j]==0)and (self.floorstatus[i+1][j+1]==0)and (self.floorstatus[i][j]==0):
                self.floorstatus[i][j]=1
    ...
    def robotinit(self):
        ...
        print("robotinit----self.robotx, self.roboty: ", self.robotx, self.roboty)
    ...
    def move(self):
        if (self.direction == 1) and (self.roboty > 0) and (self.floorstatus[self.robotx - 1][self.roboty] != 3):
            self.change()
            self.robotx -= 1
            photoname = "rbnorth.bmp"
            self.modify(self.robotx, self.roboty, photoname)
            
        elif (self.direction == 2) and(self.roboty < 7) and(self.floorstatus[self.robotx][self.roboty + 1] != 3):
            self.change()
            self.roboty += 1
            photoname = "rbeast.bmp"
            self.modify(self.robotx, self.roboty, photoname)

        elif (self.direction == 3) and (self.robotx < 7) and(self.floorstatus[self.robotx + 1][self.roboty] != 3):
            self.change()
            self.robotx += 1
            photoname = "rbsouth.bmp"
            self.modify(self.robotx, self.roboty, photoname)

        elif (self.direction == 4) and (self.roboty > 0) and(self.floorstatus[self.robotx][self.roboty - 1] != 3):
            self.change()
            self.roboty -= 1
            photoname = "rbwest.bmp"
            self.modify(self.robotx, self.roboty, photoname)
